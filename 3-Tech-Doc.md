...
+ نکته‌ی قابل توجه در مورد کلاینت، این است که برای هر کدام از نیروها، چه مورچه‌ها و چه عقرب‌ها، یک بار به طور جداگانه کد کلاینت اجرا می‌شود. در نتیجه نیروها حافظه‌ی مشترکی با یکدیگر ندارند و تنها راه ارتباطی آن‌ها از طریق چت باکس می‌باشد (توضیحات چت‌باکس را در مستند بازی بخوانید).
...
آرگومان سوم، آدرس فایل جیسون مپ را مشخص می‌کند تا بازی روی آن نقشه اجرا شود. اگر مپ تعیین نشود، بسته به مقدار متغیر READ_MAP_FROM_FILE در فایل map.config، یک نقشه برای بازی در نظر گرفته می‌شود. اگر مقدار متغیر true باشد، سرور در دایرکتوری خود به دنبال فایل مپ می‌گردد و اگر مقدار این متغیر false باشد، یک مپ رندم جنریت و بازی با آن انجام می‌شود.  
